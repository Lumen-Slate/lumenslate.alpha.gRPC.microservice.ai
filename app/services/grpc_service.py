...
class AIService(ai_service_pb2_grpc.AIServiceServicer):
    ...
    def Agent(self, request, context):
        self.logger.info(f"[Agent] Request: userId={request.userId}, role={request.role}, fileType={request.fileType}, file={bool(request.file)}, message={request.message}, createdAt={request.createdAt}, updatedAt={request.updatedAt}")
        try:
            import asyncio
            try:
